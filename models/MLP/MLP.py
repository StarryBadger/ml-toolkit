...
class MLP:
    ...
    def fit(
        ...
        if self.task == "classification":
            ...
            y_validation = (
                np.array([self.label_map[label] for label in y_validation])
                if y_validation is not None
                else None
            )
            y_train = self._one_hot_encode(y_train, self.output_size)
            y_validation = (
                self._one_hot_encode(y_validation, self.output_size)
                if y_validation is not None
                else None
            )
        else:
            ...
            print(y_train.shape)
            y_validation = (
                np.expand_dims(y_validation, axis=1)
                if y_validation is not None
                else None
            )
        ...
        for epoch in range(max_epochs):
            ...
            y_pred_validation = (
                self.predict(X_validation) if X_validation is not None else None
            )

            if self.task == "regression":
                train_metrics = Metrics(y_train, y_pred_train, task="regression")
                validation_metrics = (
                    Metrics(y_validation, y_pred_validation, task="regression")
                    if y_validation is not None
                    else None
                )
                ...
                if epoch % self.print_every == 0 or epoch == max_epochs - 1:
                    print(
                        f"Epoch {epoch}, Train Loss: {current_loss:.4f}, Train MSE: {train_mse:.4f}"
                    )
                    if validation_metrics:
                ...
                train_metrics = Metrics(
                    y_train.argmax(axis=1), y_pred_train, task="classification"
                )
                validation_metrics = (
                    Metrics(
                ...
                if epoch % self.print_every == 0 or epoch == max_epochs - 1:
                    print(
                        f"Epoch {epoch}, Train Loss: {current_loss:.4f}, Train Accuracy: {train_accuracy:.4f}"
                    )
                    if validation_metrics:
