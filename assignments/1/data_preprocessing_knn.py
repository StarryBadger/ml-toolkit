...
def drop_unnamed_columns(df):
    ...
    df.to_csv("data/interim/1/spotify/step1_drop_unnamed.csv", index=False)
    return df
...
def drop_non_numerical(df):
    ...
    df.to_csv("data/interim/1/spotify/step2_drop_non_numerical.csv", index=False)
    return df

# ? https://www.w3schools.com/python/pandas/ref_df_drop_duplicates.asp
def drop_duplicates(df):
    df = df.drop_duplicates()
    df.to_csv("data/interim/1/spotify/step3_drop_duplicates.csv", index=False)
    return df
...
def split(X, train_ratio=0.8, val_ratio=0.1):
    ...
    split_folder="data/interim/1/spotify/split"
...
def normalize(df, scaler="standard"):
    ...
    df_normalized.to_csv("data/interim/1/spotify/step4_normalized.csv", index=False)
